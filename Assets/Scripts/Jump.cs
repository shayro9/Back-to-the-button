...
public class Jump : MonoBehaviour
{
    ...
    public float grounded_remmber = 0;
    ...
    void Awake()
    {
        ...
        mask = LayerMask.GetMask("Ground","Button","Door","Platform");
    }
    ...
    void FixedUpdate()
    {
        ...
        if (isPlayer)
        {
            ...
            if (jump_pressed > 0 && canDoubleJump && unlockDoubleJump)
            {
                jumpStartHight = gameObject.transform.position.y;
                jump_pressed = 0;
                grounded_remmber = 0;
    ...
    private bool isGrounded (LayerMask m)
    {
        Vector3 position = new Vector3();
        if (gameObject.GetComponent<RightLeft>().facingRight)
            position = new Vector3(transform.position.x + collider_offset.x , transform.position.y + collider_offset.y );
        else
            position = new Vector3(transform.position.x - collider_offset.x , transform.position.y + collider_offset.y );
        ...
        ////////////////////////////Debug///////////////////////////////////////
        Debug.DrawRay(position, Vector3.down * (colliderHeight + ground_ray_len), Color.green);
        Debug.DrawRay(positionLeft, Vector3.down * (colliderHeight + ground_ray_len), Color.green);
        Debug.DrawRay(positionRight, Vector3.down * (colliderHeight + ground_ray_len), Color.green);

        return Physics2D.Raycast(position, Vector3.down, colliderHeight + ground_ray_len, m) ||
            Physics2D.Raycast(positionRight, Vector3.down, colliderHeight + ground_ray_len, m) ||
            Physics2D.Raycast(positionLeft, Vector3.down, colliderHeight + ground_ray_len, m);
    }
