...
public class RightLeft : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        mask = LayerMask.GetMask("Ground","Side_Button");
    ...
    private bool isWallAhead()
    {
        ...
        cols.Add(Physics2D.Raycast(position, Vector3.right * facing_factor, colliderWidth + 0.25f, mask).collider);
        cols.Add(Physics2D.Raycast(positionDown, Vector3.right * facing_factor, colliderWidth + 0.25f, mask).collider);
        cols.Add(Physics2D.Raycast(positionUp, Vector3.right * facing_factor, colliderWidth + 0.25f, mask).collider);
        if (cols.Count != 0)
        {
